...
import { Repeat, Signal, Radio, Plus, Home } from 'lucide-react';

export function ExtenderSuccessPage() {
      ...
      setMessages([
        {
          id: '1',
          type: 'assistant',
          content: t('extender.success.message'),
          timestamp: new Date(),
        },
      ]);
      setIsTyping(false);
  ...
  const stats = [
    { icon: Repeat, label: t('extender.success.device'), value: roomLocation || 'Extender-1' },
    { icon: Signal, label: t('extender.success.signal'), value: 'Excellent' },
    { icon: Radio, label: t('extender.success.coverage'), value: 'Extended' },
  ];
      ...
      <div className="min-h-[calc(100vh-5rem)] flex flex-col py-8">
        ...
        {showActions && (
          <div className="space-y-3 mt-8">
            <ActionCard
              icon={Plus}
              title={t('extender.success.add.another')}
              onClick={() => navigate('/extender/step1')}
              variant="primary"
            />
            <ActionCard
              icon={Home}
              title={t('extender.success.finish')}
              onClick={() => navigate('/')}
              variant="secondary"
            />
          </div>
